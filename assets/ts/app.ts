...
class TaskManager {
  ...
  getDueStatus(dateStr: string): "overdue" | "soon" | "normal" {
  ...
  renderColumn(status: Task["status"], containerId: string) {
    ...
    filteredTasks.forEach((task) => {
      const actualIndex = this.tasks.findIndex(
  ...
  prepareForAdd() {
    ...
    if (submitBtn) submitBtn.innerText = "Add Task";
...
form.addEventListener("submit", (e) => {
  ...
  const taskData: Task = {
    ...
    createdAt:
      taskManager.currentEditIndex !== null
